        ...
        if (tracking is not null)
        {
            ...
            messageText.Append(eLanguage == ELanguage.Uzbek
                ? $"Hujjat raqami: {tracking.DocNum ?? ""}\n" +
                  ...
                  $"Operatsiya sanasi: {(tracking.U_DateOfOperation is not null ? DateTime.ParseExact(tracking.U_DateOfOperation!,"yyyyMMdd", CultureInfo.InvariantCulture).ToString("dd.MM.yyyy") : "")}\n" +
                  ...
                  $"Taxminiy etkazib berish muddati: {(tracking.U_DispatchPlan is not null ? DateTime.ParseExact(tracking.U_DispatchPlan, "yyyyMMdd", CultureInfo.InvariantCulture).ToString("dd.MM.yyyy") : "")}\n" +
                ...
                : $"Номер документа: {tracking.DocNum ??  ""}\n" +
                  ...
                  $"Дата операции: {(tracking.U_DateOfOperation is not null ? DateTime.ParseExact(tracking.U_DateOfOperation!,"yyyyMMdd", CultureInfo.InvariantCulture).ToString("dd.MM.yyyy") : "")}\n" +
                  ...
                  $"Расчетный срок доставки: {(tracking.U_DispatchPlan is not null ? DateTime.ParseExact(tracking.U_DispatchPlan, "yyyyMMdd", CultureInfo.InvariantCulture).ToString("dd.MM.yyyy") : "")}\n" +
                  $"Дата отправки: {(tracking.U_DateSending is not null ? DateTime.ParseExact(tracking.U_DateSending, "yyyyMMdd", CultureInfo.InvariantCulture).ToString("dd.MM.yyyy") : "")}\n");
