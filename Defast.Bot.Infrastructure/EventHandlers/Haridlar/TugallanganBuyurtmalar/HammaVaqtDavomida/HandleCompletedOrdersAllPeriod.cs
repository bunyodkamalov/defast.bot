...
namespace Defast.Bot.Infrastructure.EventHandlers.Haridlar.TugallanganBuyurtmalar.HammaVaqtDavomida;

public class HandleCompletedOrdersAllPeriod(IBusinessPartnerService businessPartnerService, IInvoicesService invoicesService, ICacheBroker cacheBroker)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!orders!.Any())
            return await telegramBotClient.SendTextMessageAsync(
                ...
                eLanguage == ELanguage.Uzbek
                    ? "Tugallangan buyurtmalar topilmadi ❌"
        ...
        else
        {
            ...
            for (int i = 1; i <= orders!.Count / 10 + 1; i++)
                inlineKeyBoardArray[i - 1] = InlineKeyboardButton.WithCallbackData($"{i}", $"completedOrdersPage_{i}");
