...
namespace Defast.Bot.Infrastructure.EventHandlers.To_lovlar.ChiqimTo_lovlarRo_yxati.BirOylik;

public class HandleOneMonthPeriodPagination(IIncomingPaymentsService incomingPaymentsService, IBusinessPartnerService businessPartnerService, ICacheBroker cacheBroker)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!await cacheBroker.TryGetAsync<List<IncomingPayment>?>("oneMonthPeriodOutgoingPayments", out var outgoingPayments, cancellationToken))
        {
        ...
        else
        {
            ...
            for (int i = 1; i <= outgoingPayments!.Count / 10 + 1; i++)
                inlineKeyBoardArray[i - 1] = InlineKeyboardButton.WithCallbackData($"{i}", $"oneMonthPeriodPage_{i}");
